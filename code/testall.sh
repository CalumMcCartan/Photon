...
# Regression testing script for Photon
...
Usage() {
    echo "Usage: testall.sh [options] [.phn files]"
...
Check() {
    ...
    Run "$CC" "-o" "${basename}.exe" "${basename}.s" "printbig.o Image.o -lm" &&
...
if [ ! -f Image.o ]
then
    echo "Could not find Image.o"
    echo "Try \"make Image.o\""
    exit 1
fi
...
then
    ...
    files="tests/test-*.phn tests/fail-*.phn"
fi
